d3.csv("page_data/timechart1data.csv").then(makeChart);
function makeChart(untreatedData) {
  ...
  const laborset = { label: "Labor's Chance to Win", data: labor, backgroundColor: '#a62621', borderColor: '#a62621', };
  const liberal = untreatedData.map(function (d) { return d.coalition; });
  const liberalset = { label: "Coalition's Chance to Win", data: liberal, backgroundColor: '#2d45a6', borderColor: '#2d45a6', };
  const hung = untreatedData.map(function (d) { return d.hung; });
  const hungset = { label: "Hung Parliament Chance", data: hung, backgroundColor: '#858585', borderColor: '#858585', };
  ...
  const myChart = new Chart(
    document.getElementById('timechart1'),
    config
  );
